...
class WordsFilling: UIViewController, UITextFieldDelegate {
    ...
    func updateImage(){
        ...
        charsTextField[rd].text = str2.capitalizedString
    }
    ...
    @IBAction func doneTouched(sender: UIButton) {
        ...
        for i in 0..<arrayIndex{
            ...
            resultStr.appendContentsOf(ch)
            }
        }
        if resultStr.capitalizedString == wordsList[wordsIndex].capitalizedString{
    ...
    func bearForward(){
        UIView.animateWithDuration(1.0, delay: 0.5, options: .CurveEaseInOut, animations: {
            self.monsterImage.transform = CGAffineTransformMakeScale(2.0, 2.0)
            }, completion: { finished in
                UIView.animateWithDuration(1.0, delay: 1.0, options: .CurveEaseInOut, animations: {
                    self.monsterImage.transform = CGAffineTransformMakeTranslation(self.view.bounds.width/4, 0)
                    //self.monsterImage.center.x -= 20
                    }, completion: { finished in
                        UIView.animateWithDuration(1.0, delay: 0.5, options: .CurveEaseInOut, animations: {
                            self.monsterImage.transform = CGAffineTransformMakeScale(1.0, 1.0)}, completion: nil)
                })
        })
    }
    func bearBackward(){
        UIView.animateWithDuration(1.0, delay: 0.5, options: .CurveEaseInOut, animations: {
            self.monsterImage.transform = CGAffineTransformMakeScale(0.5, 0.5)
            }, completion: { finished in
                UIView.animateWithDuration(1.0, delay: 0.5, options: .CurveEaseInOut, animations: {
                    self.monsterImage.transform = CGAffineTransformMakeScale(1.0, 1.0)}, completion: nil)})}
